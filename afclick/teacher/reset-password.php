...
if(isset($_POST['change']))
{
	if($_SESSION['endtime'] > date("d-m-Y h:i:sa"))
    {
        ...
        {
            $cno=$_SESSION['teacherid'];
			$email=$_SESSION['email'];
			$newpassword=md5($_POST['password1']);
			$query=mysqli_query($con,"update teacher set t_password='$newpassword' where teacher_id='$cno' and t_email='$email'");
			if ($query) 
			{
				$subject = "Vidyalankar | Teacher Reset Password"; 
				$txt = "<h1> Vidyalankar | Teacher Reset Password </h1><br>";
...
if(isset($_POST['resend']))
{
    ...
    $addtime = strtotime("+1 minutes", strtotime($selectedTime));
    $_SESSION['endtime']=date('d-m-Y h:i:sa', $addtime);
    $message="<p class='w3-text-green'>Sucessfully send OTP to your mail.</p>";
    $rno=$_SESSION['otp'];
	$to=$_SESSION['email'];
	$subject = "Vidyalankar | Teacher password recovery"; 
		$txt = "<h1> Vidyalankar | Teacher password recovery </h1><br>";
